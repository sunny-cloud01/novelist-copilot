...
def _run_extraction_with_deep_analysis(store, book_id: str, tag: str) -> str:
    """Create + complete an extraction run carrying deep_analysis for one book."""
    run = store.create_extraction_run(book_id, trace_id=f"trace-{tag}")
    dispatched = store.mark_task_dispatched(run["task"]["task_id"], trace_id=f"trace-{tag}")
    chapter_id = store.list_book_chapters(book_id)[0]["chapter_id"]
    workspace_id = store.STORE.books[book_id]["workspace_id"]
    scene_id = f"{tag}-scene-1"
    scene = {
        "schema_version": 1,
        "scene_id": scene_id,
        "book_id": book_id,
    ...
    edge = {
        "schema_version": 1,
        "edge_id": f"{tag}-edge-1",
        "book_id": book_id,
        "workspace_id": workspace_id,
        "source_ref": f"object://knowledge-objects/{tag}-a",
        "target_ref": f"object://knowledge-objects/{tag}-b",
        "source_id": f"{tag}-a",
        "target_id": f"{tag}-b",
        "relation_type": "ally",
        "confidence": 0.8,
    ...
    store.apply_task_execution_result(
        ...
        [run["knowledge_package_ref"], run["graph_package_ref"], run["extraction_report_ref"], run["quality_report_ref"]],
        {
            ...
            "deep_analysis": {
                ...
                "events_by_scene": {},
                "relationship_edges": {edge["edge_id"]: edge},
            },
        },
...
def test_commit_knowledge_package_isolates_books_across_extractions():
    """Extracting book B must not overwrite book A's scenes/relationships.

    Regression: apply_task_execution_result used to deep-copy replace whole
    deep-analysis tables, so extract A -> extract B -> commit A leaked B's data.
    """
    from app.core import phase_two_store as store
    store.reset_store()
    store.seed_phase_two_demo_data()

    book_a = store.create_book(
        {"title": "Book A", "author_name": "A", "source_type": "reference_novel",
         "source_text": "第一章 A\nAAA。"},
        trace_id="trace-iso-a",
    )["book_id"]
    book_b = store.create_book(
        {"title": "Book B", "author_name": "B", "source_type": "reference_novel",
         "source_text": "第一章 B\nBBB。"},
        trace_id="trace-iso-b",
    )["book_id"]

    run_a = _run_extraction_with_deep_analysis(store, book_a, "aaa")
    _run_extraction_with_deep_analysis(store, book_b, "bbb")

    result = store.commit_knowledge_package(run_a)
    assert result["status"] == "succeeded"
    package = store.STORE.knowledge_packages[run_a]

    scene_books = {scene["book_id"] for scene in package["scenes"]}
    rel_books = {edge["book_id"] for edge in package["relationships"]}
    assert scene_books == {book_a}, scene_books
    assert rel_books == {book_a}, rel_books
    assert any(scene["scene_id"] == "aaa-scene-1" for scene in package["scenes"])
    assert any(edge["edge_id"] == "aaa-edge-1" for edge in package["relationships"])
    # Book B's data must still be present in the store (not overwritten by A either).
    assert any(
        edge.get("book_id") == book_b for edge in store.STORE.relationship_edges.values()
    )
